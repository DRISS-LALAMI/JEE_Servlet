...
public class Stock {
	static String [] [] leStock = {
			...
			out.println("</table> </form>");
				...
				public static String[] chercheNom (String nomDisque) {
				if(nomDisque!=null) {
					for(int i=0;i<leStock.length;i++) {
						if(leStock[i][0].equals(nomDisque)) {
							return leStock[i];
						}
					}
				}
				...
				public static String[] chercheCode (String codeDisque) {
					if(codeDisque!=null) {
						for(int i=0;i<leStock.length;i++) {
							if(leStock[i][2].equals(codeDisque)) {
								return leStock[i];
							}
						}
					}
